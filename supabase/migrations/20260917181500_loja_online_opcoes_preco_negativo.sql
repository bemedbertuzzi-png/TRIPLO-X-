-- Tamanhos como MINI custam MENOS que o produto base. Para manter o preco de
-- vitrine igual ao do cardapio impresso (o normal), a opcao "Mini" precisa ter
-- valor negativo. O piso passa a ser garantido dentro de loja_criar_pedido,
-- que recusa qualquer item cujo preco unitario final fique em zero ou abaixo.
alter table loja.opcoes drop constraint if exists opcoes_preco_centavos_check;
...
begin
  ...
  loop
    ...
    -- piso de seguranca: opcoes de desconto nunca podem zerar ou inverter o preco
    if v_unitario <= 0 then
      raise exception 'PRECO_INVALIDO: combinacao de opcoes invalida para %', v_prod.nome;
    end if;
